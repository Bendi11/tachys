...
impl FontStorage {
    ...
    pub fn load<'a, P: AsRef<std::path::Path>>(&'a mut self, path: P) -> Result<&'a [u8], FontError> {
        ...
        let storage = match unsafe { Mmap::map(&file) } {
            ...
            Err(e) => match e.kind() {
                std::io::ErrorKind::Unsupported => {
                    ...
                    let mut buf = Vec::with_capacity(file.metadata().map(|m| m.len() as usize).unwrap_or(65536));
